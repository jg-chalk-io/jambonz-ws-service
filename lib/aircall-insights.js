...
class AircallInsights {
  ...
  static async getAircallCallId(phoneNumber) {
    ...
    try {
      // Strip + prefix from phone number - Aircall database stores without it
      const phoneNumberWithoutPlus = phoneNumber.replace(/^\+/, '');

      // Only consider calls from the last 10 seconds to avoid grabbing old calls
      const tenSecondsAgo = new Date(Date.now() - 10000).toISOString();

      logger.info({phoneNumber, phoneNumberWithoutPlus, tenSecondsAgo}, 'Searching Aircall Supabase database for call ID');

      const {data, error} = await aircallSupabase
        .from('aircall_data')
        .select('"Call id", "Call start time", direction, "from"')
        .eq('"from"', phoneNumberWithoutPlus)
        .eq('direction', 'inbound')
        .gte('"Call start time"', tenSecondsAgo)
        .order('"Call start time"', {ascending: false})
        .limit(1);
      ...
      const call = data[0];
      const aircallCallId = call['Call id'];

      logger.info({
        phoneNumber,
        aircallCallId,
        callStartTime: call['Call start time']
