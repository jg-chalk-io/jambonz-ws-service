...
class AircallInsights {
  ...
  /**
   * Get Aircall Call ID by searching for the most recent inbound call
   * from the given phone number.
   *
   * When using Twilio SIP trunk, the Ring-to widget provides the Twilio
   * Call SID, not the Aircall Call ID. This function queries the Aircall
   * Search Calls API to find the real Aircall Call ID.
   *
  ...
  static async getAircallCallId(phoneNumber) {
    return new Promise((resolve) => {
      ...
      // Search for most recent inbound call from this number
      ...
      const options = {
        hostname: 'api.aircall.io',
        port: 443,
        path: `/v1/calls/search?${queryParams}`,
        method: 'GET',
        headers: {
          'Authorization': `Basic ${auth}`,
          'Content-Type': 'application/json'
      ...
      logger.info({phoneNumber}, 'Searching Aircall for call ID');
          ...
          try {
            ...
            if (res.statusCode !== 200) {
              logger.error({
                ...
                response: parsed,
                phoneNumber
              }, 'Aircall Search Calls API error');
              return resolve(null);
            }

            // Get the most recent call (first result)
            const calls = parsed.calls || [];

            if (calls.length === 0) {
              logger.warn({phoneNumber}, 'No calls found in Aircall search');
              return resolve(null);
            }

            const mostRecentCall = calls[0];
            const aircallCallId = mostRecentCall.id;

            logger.info({
              phoneNumber,
              aircallCallId,
              callsFound: calls.length,
              callDirection: mostRecentCall.direction,
              callStatus: mostRecentCall.status
            }, 'Found Aircall Call ID');

            resolve(aircallCallId.toString());
          } catch (err) {
            logger.error({err, responseData}, 'Failed to parse Aircall search response');
            resolve(null);
          }
        });
      });
      ...
      req.on('error', (err) => {
        logger.error({err, phoneNumber}, 'Aircall Search Calls API request failed');
        resolve(null);
      });

      req.setTimeout(2000, () => {
        logger.error({phoneNumber}, 'Aircall Search Calls API timeout');
        req.destroy();
        resolve(null);
      });
