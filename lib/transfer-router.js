...
function determineTransferRoute(transferNumber, client) {
  ...
  if (normalizedAircall && normalizedTransfer === normalizedAircall) {
    // Ensure E.164 format for phone number (+1XXXXXXXXXX)
    ...
    return {
      type: 'aircall_sip',
      method: 'twilio_sip',
      destination: e164Number,  // Use E.164 phone number with byoc attribute
      sipUri: null,  // Not using SIP URI - using Number with byoc instead
